...
Map clusters show a representative **word +x**, where x counts the other forms, not languages. The representative is the form closest to the cluster's centre, with a stable code/term tie-break. Click the word to read its etymology in the sidebar. Hover the card to preview the other forms in a downward stack; click +x (or focus it and press Enter) to keep the stack open. Each form has a selection button, language, available readings, and a separate ↗ Wiktionary link. Escape, clicking elsewhere, or moving the map dismisses the stack. Large groups scroll without zooming the map and render 30 additional rows at a time, with no new network requests just for hovering. This is a geographic representative, not an etymological-confidence ranking.
...
[Coverage expansion candidates and integration requirements](data/COVERAGE_PLAN.md): Lexibank, IDS, WOLD, and structured Wiktionary extraction. These are researched next steps, not sources already imported into the map.
